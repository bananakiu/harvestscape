...
const MAP_ACCESS = {
  ...
  green: "★ v6.3 — east off the village's south lane, through a gap in the hedge. The valley's old festival grounds, and the second promise the game made before it had a place to keep it: a weekly writ named THE FESTIVAL GREEN that the player could already fund and close ('trestles, canopy, and a table long enough for everybody'), and Maya's 3♥ line about a bench she saved here. Both land on this map. Before the writ it is a leaning flagpole, a cold fire-ring and four bald patches where the trestles stood; after it, the tables are back, the canopy is up and the flag flies. The Egg Fair and the Harvest Fair are held here (see FESTIVAL_VENUE) — an egg hunt wants long grass and a crop judging wants trestle tables; the Luau and the Star-Watch keep the sea.",
  marrowpoint: "★ v6.2 — thirty-nine miles up the coast, reached by the ferry from the coast road's mooring. The most pre-written place in the game before it existed: a milestone carved MARROW POINT — 39 the player could already examine, two signposts, a ferry sign reading 'no service', and a lighthouse that has been blinking in the ridge cairn's panorama since v3.43. Elias worked its ferry for eleven years and never once made the crossing in this direction; the first sailing is his. The light, the quay, the ferry-master's hut, headland forage (sea holly, samphire, coral, driftwood) and the long grey water.",
  ...
  undercroft: "Behind the planked tenth door in the Guild, opened by 'The Tenth Door' (Act III; needs Act II done + total level 100). The Warden's wing, sealed eleven years — floors 1–30 of cozy-dark cavern, knotted with restless things you SETTLE with the Stave (never kill; nothing is ever taken, knockout costs zero). Five families band by depth (Gloam Wisp → Knot-Shambler → Ember Mite → Hollow Warden, which guards its front → Gloam Tangle, which splits), and every 10th floor a Great Knot boss guards the descent. The way down hides under a knot (or the boss) on each floor; a Warden's Bell every 5th floor is a Pledge-Ledger checkpoint you can ring back down to. Time stands still, like the mine. The only hazardous space in the valley — and only because you walk into it.",
  ...
  butterbrook: "West off the beach, the coast opens south (v3.44). Shore-meadows, the brook to the sea under a plank footbridge, and the creamery at the far west — the longest walk in the valley. Home of Nell, the coast dairy keeper (Tom's wife), who buys dairy goods — milk, cheese, wool, eggs — at a daily premium order, closing the barn→coast→cheese loop. Samphire forages on the sand.",
  dairy: "Inside the Butterbrook creamery. Nell works here by day (the meadow of an evening). Talk to her to fill today's dairy order for coin, Farming XP, and her regard.",
...
function skillsSection(){
  ...
  const wardRows = Object.values(D.CREATURES || {}).map(c => {
    ...
    const loot = c.drop ? ` · drops ${esc(c.drop)} (${D.ITEM_SELL[c.drop]}g)` : ` · splits rather than dropping`;
    const xp = c.xp ? `settle it (never kill) for ${c.xp} Warding XP` : `settle it (never kill)`;
    return [c.lvl, `<b>${esc(c.name)}</b>${tag} — ${xp}${loot}`];
  });
