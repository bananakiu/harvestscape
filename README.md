...
- **A world to explore** — a farm overworld plus **enterable interiors** (your cottage, the coop, Tom's store, Maya's house, the Guild Hall), a **procedural multi-floor mine** (ore, gems, crystals, and a sealed Star-Metal vault deep down), a **coast** (better fishing, shore forage, and the festival grounds), and — new in v4 — the **Undercroft** behind the Guild's tenth door: a cozy-dark combat wing you settle rather than fight. Doors, ladders, and paths warp you between them with a smooth fade.
- **Cozy, opt-in combat (v4 "The Tenth Door" → "Deeper Still")** — **Warding**, a sixth 1→99 skill with a restless thing to settle at every rung of the ladder: down **forty-five floors** of the Undercroft you meet gloam wisps, knot-shamblers, ember mites, the **Hollow Warden** that guards its front (circle it), the **Gloam Tangle** that splits when struck, the charging **Deep Knot** (dodge and it slams the wall and stuns itself), and the **Star-Gnarl** — the game's first **ranged** enemy, lobbing star-bolts you sidestep — while every tenth floor a **Great Knot** boss guards the descent. You **settle** them — not slay — with a forged Warden's Stave. Every attack is telegraphed, health bars and numbered hitsplats read the fight, and defeat is a soft knockout that wakes you safe at the door with **everything still yours** (nothing is ever taken — the cozy contract holds). It lives only in the Undercroft; the whole rest of the valley stays hazard-free. Settling drops feed charms (up to the +15-Resolve Starward) and Pledge-Ledger bell checkpoints; a daily **variety spark** rewards rotating your crafts.
...
- **RuneScape-style progression, paced to be savored** — 6 skills trained 1→99 on a bespoke XP curve (Farming, Woodcutting, Mining, Fishing, Cooking, and now Warding; early levels earned, not showered; a long steady climb; only the final stretch a true completionist mastery award — no 13-million-XP wall), level-gated content (crops, trees, ores, fish, a full Cooking recipe ladder, and the Undercroft's depths), mastery perks at 25/50/75/99 — and when you hit one, the neighbour who cares most about that craft says a warm word. Seven tiers of upgradeable tools (Basic → Copper → Iron → Gold → Cobalt → Deepsilver → Star Metal), the Warden's Stave among them.
...
- `game/` — the finished game (`index.html`, `css/`, `js/` split into 16 ordered modules, `00-core` → `13-content` → `15-warding` → `10-ui` → `11-title` → `14-story` → `12-game`).
