...
| **v4.4.0 "Hold the Line"** (SHIPPED) | The **Warden's Guard** — a block/parry for Undercroft combat (owner report: the Hollow Warden could only be run from). A timed guard (Shift / right-click in the Undercroft / touch 🛡) that turns aside a facing blow: a **parry** (caught in the opening beat) negates all Resolve loss, staggers the attacker, and knocks a Hollow Warden's guarded front open for a riposte; a **block** (late) absorbs ¾. Free, cooldown-gated, one-hit-per-press, facing-required — skillful but cozy (only ever saves Resolve). Combat now has offense (Stave) **and** defense (Guard). |
| **v4.5.0 "The Tenth Lantern"** (SHIPPED) | **Act III COMPLETE.** The Warden's Ledger's five remaining chapters + the finale (ch4–8), authored into the v4.3 engine: past where Elias kept (f20), Rowan comes down to the wing he sealed (f25), the last warden Nell's name (f30), the deepest knot (f40), and **the Tenth Lantern** (f45) — the tenth craft lit and counted, the tenth door glowing for good, the Guild hall ablaze, the valley made truly whole. GBP-honest deep bundles; the deep grind finally has a story to be the point of. |
| **mastery trials at 50** (remaining v4.1 story) | The 50-gate trial per skill (all six), each set by the craft's caring NPC (`MASTERY_NPC` maps all six). The flesh-out audit's #2 pick. **Must be bank-and-release** (level HOLDS at 50/75 while XP banks, never regresses) and **grandfather existing high-level saves** (auto-pass tiers already past) — a naive "level drops until you clear it" reading would take a level and break the cozy contract. Deferred from v4.3/v4.4/v4.5 — touches every skill's progression, wants its own release. |
