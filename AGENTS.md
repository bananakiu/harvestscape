...
### ★ The checks (v5.0 "The Strongbox", extended v6.4.3) — run these, they are fast and they are the only automation

The game has no test runner and no build step, deliberately. These six node scripts are what
...
| `node tools/check-perf.mjs` | ★ **Run it alone, on a quiet machine** — chaining it behind the other checks and the atlas produced a spurious single-measurement breach (v6.4.4); four solo runs were all clean. A lone breach is a prompt to re-run, then to look. The year-3 budget: `migrateSave`, save serialize, save size, map generation, export round-trip — against recorded ceilings in `tools/fixtures/perf-budget.json`. `--set` re-records after an intentional change. |
...
| `node tools/check-schedules.mjs` | ★ **Run after ANY change to `spawnMapNpcs` or a wander box.** Sweeps every NPC × every map × 13 hours × 28 days: nobody in two places at once, every NPC reachable in a week, and no two wander boxes close enough to collide their name tags (measured by simulating 400 steps, not by comparing rectangles). Four double-bookings and two crowding bugs were invisible in a diff and obvious to this. |
| `node tools/check-forage.mjs` | ★ **Run after ANY change to WILD, PREPS or WILD_CATS.** 148 invariants sampled against the live generators: R1 every preparation makeable on ≥40% of the days of every season (the WEATHERS contract as a test), R2 every input gatherable ≥6 game hours, R3 no orphans. Also prints the spawn census, which is how four spawner bugs were found before release rather than after. |
| `node tools/check-interactions.mjs` | ★ **Run before ANY release.** Stands the player next to every object on every map and presses E, then Q — 1,934 presses, 84 object kinds. Catches the class node's syntax check cannot: a legal identifier that is not bound, in a branch nothing else exercises. Two shipped bugs prompted it (every bench in the game dead for three releases; the Hammer unholdable), and neither was visible to the other four checks — none of which press a key. |
| `node tools/build-atlas.mjs` | The atlas (already required every content change). Now also renders the **unlock-cadence table** — every ≥8-level band that unlocks nothing, XP-weighted. Also fails when a Guild wing's `lit()` condition changes, so its hand-written prose cannot go stale. |
